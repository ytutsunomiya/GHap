---
title: "The **GHap** Package (Version 4.0.0beta)"
author: "Yuri Tani Utsunomiya"
date: "18 Nov 2022"
output:
  pdf_document:
    highlight: rstudio
...
In the v3 release, the source code was extensively modified for improved performance and inclusion of new functionality for both phased and unphased SNP data, including analysis of runs of homozygosity, sampling methods for virtual gamete mating, mixed model fitting and GWAS.

For the v4 release, GHap will allow phased genotypes to be organized as a matrix of individuals x variants in the input file, apart from the default variants x individuals. The source code will also integrate C++ to boost performance through Rcpp. New functionality will also be incorporated.

Overall, the package is a comprehensive and versatile tool for the analysis of genomic data. Please notice that this specific version of the package is a development (beta) version, so some functions may still contain bugs and behave unexpectedly.
...
Since the v4 release, the **.phase** file can be alternatively organized as a $n$ x 2$m$ matrix of phased genotypes, i.e., each individual in a single row, and each phased marker represented by two columns (one for each haplotype). The `ghap.compress` function will then print a "magic number" to the **.phaseb** file during conversion, namely 00000001 for the original GHap format (variant x individual) and 00000010 for the new format (individual x variant). For backward compatibility, GHap also considers the (variant x individual) **.phaseb** file without a leading byte.
...
If variance components are estimated from the data, standard errors and confidence intervals of any arbitrary function of variance components can be approximated using the sampling-based method implemented in the `ghap.remlci()` function. For example, if one wishes to estimate standard errors and confidence intervals for the heritability, the following code can be used:
...
The `ghap.assoc()` function implements methods for Genome-Wide Association Studies (GWAS). The example below shows how to perform GWAS in **GHap**:
